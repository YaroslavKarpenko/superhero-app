  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        await api.addSuperhero(formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });

        alert('Superhero successfully added.');
        dispatch(initializeSuperheroes({ page: currentPage, limit: 5 }));
        setFormIsVisible(false);
        resetForm();
      } catch (error) {
        console.error('Error superhero:', error);
        alert(`Failed to add superhero. Please try again. ${error.response.data}`);
      }
    },
  });
    ...
    <div className="flex flex-col justify-center items-center ">
      {formIsVisible ? (
        <button
          ...
          className="flex h-8 w-fit items-center gap-1 border-2 border-gray-200 border-solid rounded-md p-2 text-gray-200 hover:text-gray-400 hover:border-gray-400 transition-all	duration-200	ease-in-o">
          <CloseIcon color="inherit" fontSize="large" />
          <span className="text-xl">Hide form</span>
        </button>
      ) : (
        <button
          onClick={() => {
          ...
          <span className="text-xl">Add new Superhero</span>
        </button>
      )}
      {formIsVisible && (
        <div className="w-[800px] mt-5 mx-auto text-gray-200">
          <form
            onSubmit={formik.handleSubmit}
            className="grid grid-cols-3 gap-4 items-center text-lg">
          ...
          <button
            ...
            className="flex justify-center mx-auto mt-8  h-8 w-fit items-center gap-1 border-2 border-gray-200 border-solid rounded-md p-2 text-gray-200 hover:text-gray-400 hover:border-gray-400 transition-all	duration-200	ease-in-o">
